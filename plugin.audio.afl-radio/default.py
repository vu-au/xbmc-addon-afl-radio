...
def get_url(station):
	#return "http://%s/online-radio-afl_%s|%s" % (HOST, station, USER_AGENT)
	return "http://%s/online-radio-afl_%s" % (HOST, station)
...
def play(params):
	...
	listitem.setInfo('audio', infoLabels=labels)
	listitem.setThumbnailImage(thumb)
	#url = "%s/%sonline-radio-afl_' + p['id'] + '?WMBitrate=262144|' + urllib.urlencode({'User-Agent':'NSPlayer/11.08.0005.0000'})
	url = get_url(p['id'])
